<?php
	...
	if($result_select_cotiz){
		...
		$pdf->Ln(); 
		$pdf->SetLeftMargin(5);
		$pdf->Cell(25,8,'                                                 Cotizacion Num 000'. $idcotiz .' - COMPRAS','C');
		$pdf->Ln();
		...
		if($result_cotiz_detalle){
			while($row_cotiz_detalle = mysqli_fetch_array($result_cotiz_detalle)){
				...
				$pdf->Cell(25,10,'$ '. moneyFormat($row_cotiz_detalle['costo_total']),1,0,'C');
		...
		$pdf->Ln(8);
		$pdf->SetFont('Arial','B',15);
		$pdf->Cell(30,8,'Costo total: $ '. moneyFormat($getCosto),'C');
		$pdf->Ln(5);
